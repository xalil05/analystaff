...
class AuditLog(Base, BigIntIdentityMixin, CreatedAtMixin):
    ...
    resultat: Mapped[Optional[dict]] = mapped_column(VARCHAR(30), nullable=True)
    context: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
    ip_address: Mapped[Optional[str]] = mapped_column(INET, nullable=True)
    created_at: Mapped[str] = mapped_column(TIMESTAMP(timezone=True), nullable=False, index=True)
